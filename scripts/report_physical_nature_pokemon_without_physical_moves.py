...
with open(settings.REBORN_DB_PATH / 'queries' / 'physical_nature_pokemon_without_physical_moves.sql') as f:
    query = f.read()
...
for row in results:
    trainer_type, trainer_name, party_id, trainer, index, pokemon, nature, moves = row
    print(f"{trainer}'s {pokemon} -", nature, '- no physical moves')
