...
def extract():
	...
	marshal_mapinfo_obj = marshal.load(settings.REBORN_DATA_PATH / 'MapInfos.rxdata')
	marshal_mapinfo_cols = ('@name', '@scroll_x', '@scroll_y', '@order', '@expanded', '@parent_id')
	marshal_mapinfo_rows = []

	for map_id, mapinfo_for_id in marshal_mapinfo_obj.mapping.items():
		mapinfo_for_id = {
			marshal.pythonify(field): marshal.pythonify(val)
			for field, val in mapinfo_for_id.vars.items()
		}
