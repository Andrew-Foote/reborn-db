...
from utils import apsw_ext, pbs, script
...
def lookup_item(db, id_):
	return list(db.cursor().execute('select "id" from "item" where "code" = ?', (id_,)))[0][0]

def lookup_move(db, id_):
	return list(db.cursor().execute('select "id" from "move" where "code" = ?', (id_,)))[0][0]

def lookup_pokemon(db, id_):
	return list(db.cursor().execute('select "id" from "pokemon" where "number" = ?', (id_,)))[0][0]
...
def handle_evolutions(db):
	...
	def get_complement_maps(map_set):
		return [map_ for map_, in db.cursor().execute(
	...
	def get_form_name(pokemon, index):
		to_form_names = list(db.cursor().execute(
	...
	with db:
		for section in pbs_data:
			...
			for form_index, form_name in list(db.cursor().execute(
				...
				if (id_, form_name) in MANUAL_EVOLUTIONS:
					...
					for evolution_id, method_id, argument in script_evolutions:
						...
						evolution_id = lookup_pokemon(db, evolution_id)
						pbs_method, parse_argument = PBS_METHODS_BY_ID[int(method_id)]
						
						evolutions.append((
							pbs_method,
							parse_argument(db, argument),
							evolution_id,
	...
	apsw_ext.bulk_insert_multi(db, evolution_rows)
